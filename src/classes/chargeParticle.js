...
export default class ChargeParticle extends NodeObj {
  ...
  eletric_field_at(target_position) {
    ...
    const mod = this.K * (this.charge / Math.pow(target_position.distance_to(this.position) * scale, 2))
  ...
  getForce() {
    ...
    for (let i of ChargeParticle.Charges) {
      ...
      chargesum += ((this.charge * i.charge) / Math.pow(this.position.distance_to(i.position)*SimulatorConfiguration.scale,2))*this.K
      dir = dir.sum_point(this.position.direction(i.position))
    }
    dir = dir.normalize()
    return dir.multiply(chargesum)
  }
